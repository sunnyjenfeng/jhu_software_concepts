# -*- coding: utf-8 -*-
"""Flask + tiny local LLM standardizer with incremental JSONL CLI output - PARALLELIZED VERSION."""
...
def _cli_process_file(
    ...
    """Process a JSON file and write JSONL incrementally with optional parallelization.
    ...
    Args:
        ...
        use_parallel: Use multiprocessing for parallel LLM calls (default True)
    """
    ...
    try:
        if use_parallel and len(rows) > 1:
            # Parallel processing with multiprocessing
            num_workers = max(1, cpu_count() - 1)  # Leave 1 for OS
            print(
                f"[Parallel Mode] Processing {len(rows)} rows with {num_workers} workers...",
                file=sys.stderr,
            )
            
            # Extract program text from rows
            program_texts = [((row or {}).get("program") or "") for row in rows]
            
            # Process in parallel
            try:
                with Pool(processes=num_workers) as pool:
                    results = pool.map(_call_llm, program_texts)
                
                # Write results with original rows
                ...
                print("[Parallel Mode] Completed successfully", file=sys.stderr)
            except Exception as e:
                print(
                    f"[Parallel Mode] Failed: {e}. Falling back to sequential...",
                    file=sys.stderr,
                )
            ...
            print("[Sequential Mode] Processing rows...", file=sys.stderr)
            for row in rows:
                program_text = (row or {}).get("program") or ""
                result = _call_llm(program_text)
                row["llm-generated-program"] = result["standardized_program"]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Standardize program/university with a tiny local LLM (parallelized).",
    )
    parser.add_argument(
        "--file",
    ...
    parser.add_argument(
        "--no-parallel",
        action="store_true",
        help="Disable multiprocessing (use sequential processing instead).",
    )
